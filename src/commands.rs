...
mod tests {
    ...
    fn test_create_version_directory() {
        ...
        assert!(env.version_exists(version), "Version directory should exist");
    }
    ...
    fn test_multiple_versions_coexist() {
        ...
        for version in &versions {
            assert!(env.version_exists(version), "Version {} should exist", version);
        }
    }
    ...
    async fn test_activate_bun_copies_binary() {
        ...
        assert_eq!(active_content, format!("mock bun {}", version), "Active binary should match source");
    }
    ...
    async fn test_remove_version() {
        ...
        let version = "1.0.0";
        env.create_mock_version(version);
        assert!(env.version_exists(version), "Version should exist before removal");
        
        let version_path = env.versions_dir.join(version);
        fs::remove_dir_all(&version_path).unwrap();
        
        assert!(!env.version_exists(version), "Version should not exist after removal");
    }
    ...
    async fn test_switch_between_versions() {
        ...
        assert_eq!(env.read_active_bun().unwrap(), format!("mock bun {}", v1), "Should activate v1");
        
        // Switch to v2
        let bun_v2 = v2_dir.join(BUN_BIN_NAME);
        fs::copy(&bun_v2, env.bin_path()).unwrap();
        assert_eq!(env.read_active_bun().unwrap(), format!("mock bun {}", v2), "Should activate v2");
    }
    ...
    async fn test_list_versions_sorted() {
        ...
        assert_eq!(found_versions, vec!["2.0.0", "1.2.0", "1.1.0", "1.0.0"], "Versions should be sorted descending");
    }
    ...
    fn test_version_with_special_characters() {
        ...
        for version in &versions {
            env.create_mock_version(version);
            assert!(env.version_exists(version), "Special version {} should exist", version);
        }
    }
    ...
    fn test_version_persistence() {
        ...
        assert!(env.version_exists(version), "Source version should persist after activation");
        ...
        assert_eq!(active, original, "Active binary content should match original");
    }
}
